...
contract PuzzleWalletTest is DSTest {
    function testFunction() public {
        bytes4 deposit_sig = bytes4(keccak256("multicall(bytes[])"));
        bytes4 multicall_sig = bytes4(keccak256("multicall(bytes[])"));
        console.logBytes4(deposit_sig);
        console.logBytes4(multicall_sig);
        // bytes[] memory data = [deposit_sig];
        bytes[] storage arr;
        bytes memory offset = new bytes(1);
        offset[0] = byte(uint8(32));
        bytes memory len = new bytes(1);
        len[0] = byte(uint8(1));
        
        arr.push(offset);
        // arr[0] = offset;
        // arr[1] = len;
        // arr[2] = abi.encodeWithSignature("deposit()");
        // console.logBytes(RLP._flatten(arr));
    }   
}